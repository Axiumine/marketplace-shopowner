...
| Coverage file audit | `scripts/coverage-audit.mjs`, `coverage-exempt.txt` | the same `yarn test:cov` exits non-zero if a source file `coverage.include` gates is absent from the report and unnamed |
