 ...
 *
 * Replies are queued per operation name, so a test can say "the first InfoAdminAfterLogin fails with
 * 498, the Refresh succeeds, the retry succeeds" and get exactly that sequence.
 */
 ...
 *
 * There is exactly one on the platform — the OpenStreetMap geocoder behind the address field — and it
 * goes through the same `fetch` this helper replaces, so it has to be answered here or it lands in the
 * queue lookup below and throws as an unconfigured operation. Returning `undefined` means "not mine",
 * and the request falls through to the GraphQL handling.
 *
...
const jsonResponse = (reply: GraphQLReply): Response =>
	new Response(JSON.stringify({ data: reply.data ?? null, errors: reply.errors }), {
...
export const graphQLError = (title: string, description?: string, status = 400) => ({
	message: title,
