...
describe('Turnstile without a site key', () => {
	 ...
	 * decides: `guardPublicLogin` verifies a token only when it holds a secret key of its own, so a
	...
	// request per page load for a widget that cannot work.
...
describe('loading the script', () => {
	 ...
	 * needs it, and a shop owner who is already signed in never sees that page at all.
	 */
...
describe('rendering the widget', () => {
	...
	/*
	 * The effect can be torn down while the script is in flight — a shop owner who navigated away from the
...
describe('when the script cannot load', () => {
	 ...
	 */
	it('tells the shop owner, out loud', async () => {
		await mount()
		await scriptFails()
