 ...
 * the normal state of a developer machine and of the test suite, and it is safe because the server is
 * the side that decides: `guardPublicLogin` on `marketplace-dev-public-authorization` verifies a token
 * only when it holds a secret key of its own, so a deployment with the secret configured rejects the
 * tokenless request this component would send. The client cannot weaken the gate by not rendering — it
